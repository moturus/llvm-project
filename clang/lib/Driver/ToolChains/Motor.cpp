...
void tools::motor::Linker::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  // The on-image toolchain lives under /devtools/llvm (headers + libs), not
  ...
  if (!Args.hasArg(options::OPT_nostdlib, options::OPT_nostartfiles,
                   options::OPT_r))
    CmdArgs.push_back(
        Args.MakeArgString(D.SysRoot + "/devtools/llvm/lib/crt1.o"));

  Args.addAllArgs(CmdArgs, {options::OPT_L, options::OPT_u});
  TC.AddFilePathLibArgs(Args, CmdArgs);
  CmdArgs.push_back(
      Args.MakeArgString("-L" + D.SysRoot + "/devtools/llvm/lib"));
  ...
  if (!Args.hasArg(options::OPT_nostdlib, options::OPT_nodefaultlibs,
    ...
    // -lc++ (the C++ standard library proper) only when compiling C++.
    if (D.CCCIsCXX() && TC.ShouldLinkCXXStdlib(Args))
      CmdArgs.push_back("-lc++");
    // -lc++abi unconditionally, even for C: mlibc is implemented in C++, so its
    // libc.a members (e.g. mlibc::fd_file's dtor, the charcode adapters)
    // reference `operator delete`/`operator new`. Those live in libc++abi, so a
    // plain C link that pulls any such member fails with
    // `undefined symbol: operator delete` without it. An unreferenced archive is
    // free, so a C program that never triggers those members pays nothing.
...
// Motor's sysroot layout is <sysroot>/devtools/llvm/include{,/c++/v1}
...
void Motor::AddClangSystemIncludeArgs(const ArgList &DriverArgs,
  ...
  addExternCSystemInclude(DriverArgs, CC1Args,
                          D.SysRoot + "/devtools/llvm/include");
}
...
void Motor::AddClangCXXStdlibIncludeArgs(const ArgList &DriverArgs,
  ...
  if (GetCXXStdlibType(DriverArgs) == ToolChain::CST_Libcxx)
    addSystemInclude(DriverArgs, CC1Args,
                     getDriver().SysRoot + "/devtools/llvm/include/c++/v1");
}
